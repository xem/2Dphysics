class Rigidbody{
	constructor(shape, mass = 1){
		...
		this.nonCollidingObjects = new Set();

	}
	
	addNonCollidingRigi(rigi){
		this.nonCollidingObjects.add(rigi);
		rigi.nonCollidingObjects.add(this);
	}
	
	removeNonCollidingRigi(rigi){
		if(this.nonCollidingObjects.has(rigi)){
			this.nonCollidingObjects.delete(rigi);
		}
		if(rigi.nonCollidingObjects.has(this)){
			rigi.nonCollidingObjects.delete(this);
		}
	}
	
	canCollide(rigi){
		return !this.nonCollidingObjects.has(rigi);
	}
