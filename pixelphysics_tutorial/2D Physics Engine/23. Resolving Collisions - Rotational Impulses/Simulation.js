class Simulation{
	constructor(worldSize){
		...
		this.rigidBodies.push(new Rigidbody(new Rectangle(new Vector2(750,250), 200,100),1));
		this.rigidBodies.push(new Rigidbody(new Rectangle(new Vector2(200,0), 200,100),1));
		let vertices = [new Vector2(500,150), new Vector2(700,200), new Vector2(750,500), new Vector2(200,350)];
		this.rigidBodies.push(new Rigidbody(new Polygon(vertices),2));
		...
		for(let i=0; i<this.rigidBodies.length;i++){
			for(let j=0; j<this.rigidBodies.length;j++){
				if(i != j){
					...
					rigiA.shape.color = "black";
					rigiB.shape.color = "black";
					...
					if(collisionManifold != null){
						...
						collisionManifold.rigiA.shape.color = "red";
						collisionManifold.rigiB.shape.color = "red";
